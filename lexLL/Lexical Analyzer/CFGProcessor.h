...
public:
    ...
    map<string,set<string>> firstValues;
    ...
    CFGProcessor(string cfgFileName){
       // cout<<"start";
        ...
        if(file.is_open()){
            ...
            parseLine(line);
            LL p =LL();
            split=p.makeLL(split);
            for(map<string,vector<string>>::const_iterator it = split.begin();
                it != split.end(); ++it){
                bool flag = false;
            ...
            for(map<string,vector<string>>::const_iterator it = split.begin();
                it != split.end(); ++it)
                first_fill(it->first);
            for(map<string,set<string>>::const_iterator it = firstValues.begin();
